...
export default function Header() {
  ...
  useEffect(() => {
    if (user) {
      notificationApi.list().then((res: any) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white border-b border-gray-200">
      <div className="px-4 sm:px-6 h-14 flex items-center justify-between">
        {/* Logo */}
        <button onClick={() => router.push('/feed')} className="text-lg font-bold text-gray-900 tracking-tight">
          Locker
        </button>
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => router.push('/verify')}
            className={`px-3 py-1.5 text-sm rounded-lg transition ${pathname === '/verify' ? 'bg-gray-900 text-white' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'}`}>
            认证
          </button>
          <button onClick={() => router.push('/my-circles')}
            className={`px-3 py-1.5 text-sm rounded-lg transition ${pathname === '/my-circles' ? 'bg-gray-900 text-white' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'}`}>
            我的圈子
          </button>
